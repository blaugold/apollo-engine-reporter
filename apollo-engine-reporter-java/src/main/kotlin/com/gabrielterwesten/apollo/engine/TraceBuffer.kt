...
class TraceBuffer(
  ...
  /**
   * Add a trace to the buffer. If the new trace brings [bufferedBytes] over [maxBufferedBytes] the
   * buffer starts discarding the oldest entries.
   */
  fun addTrace(signature: String, trace: Trace) =
      synchronized(this) {
        ...
        if (discardedEntries.isNotEmpty()) log.warn("Discarded ${discardedEntries.size} entires.")
      }
  ...
  private data class Entry(val signature: String, val trace: Trace)
}

...
abstract class AbstractHeaderFilterProcessor(
  ...
  private val replacementStrategy: HeaderReplacementStrategy =
      replacementStrategy ?: DefaultHeaderReplacementStrategy()

  override fun processHeaders(headers: Map<String, List<String>>): Map<String, List<String>> {
    val replacementValues =
        headers.filter { !includeHeader(it.key, it.value) }.mapValues {
          replacementStrategy.replacementValue(it.key, it.value)
        }

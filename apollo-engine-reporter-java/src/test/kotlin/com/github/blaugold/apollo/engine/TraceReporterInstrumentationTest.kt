...
internal class TraceReporterInstrumentationTest {
    ...
    fun `support removing tracing data from extension`() {
        // Given
        val instrumentation = TraceReporterInstrumentation(TestTraceReporter(), removeTracingData = true)
        val graphQL = testGraphQL(instrumentation)

        // When
        val executionResult = graphQL.execute {
            it.query("query { hello }")
        }
    ...
    fun `support leaving tracing data from extension`() {
        // Given
        val instrumentation = TraceReporterInstrumentation(TestTraceReporter(), removeTracingData = false)
        val graphQL = testGraphQL(instrumentation)
    ...
    private fun testGraphQL(testInstrumentation: TraceReporterInstrumentation,
                            includeTracingInstrumentation: Boolean = true): GraphQL {
        val schema = """
            type Query {
                hello: String
            }
